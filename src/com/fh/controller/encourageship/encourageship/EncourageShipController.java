package com.fh.controller.encourageship.encourageship;
...
import com.fh.service.encourageship.encourageship.EncourageShipManager;
 ...
 */
@Controller
@RequestMapping(value="/encourageship")
public class EncourageShipController extends AcStartController {
	
	String menuUrl = "encourageship/list.do"; //菜单地址(权限用)
	@Resource(name="encourageshipService")
	private EncourageShipManager encourageshipService;
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增EncourageShip");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("ENCOURAGESHIP_ID", this.get32UUID());	//主键
		encourageshipService.save(pd);
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除EncourageShip");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		encourageshipService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改EncourageShip");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		encourageshipService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表EncourageShip");
		...
		page.setPd(pd);
		List<PageData>	varList = encourageshipService.list(page);	//列出EncourageShip列表
		mv.setViewName("encourageship/encourageship/encourageship_list");
	...
	public ModelAndView listApplyFlow(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = encourageshipService.list(page);	//列出EncourageShip列表
		mv.setViewName("encourageship/encourageship/encourageship_list_apply_flow");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		mv.addObject("QX",Jurisdiction.getHC());	//按钮权限
		return mv;
	...
	public ModelAndView listUpFlow(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = encourageshipService.list(page);	//列出EncourageShip列表
		mv.setViewName("encourageship/encourageship/encourageship_list_up_flow");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		mv.addObject("QX",Jurisdiction.getHC());	//按钮权限
		return mv;
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("encourageship/encourageship/encourageship_edit");
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = encourageshipService.findById(pd);	//根据ID读取
		mv.setViewName("encourageship/encourageship/encourageship_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除EncourageShip");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			encourageshipService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出EncourageShip到excel");
		...
		List<PageData> varOList = encourageshipService.listAll(pd);
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var13", varOList.get(i).getString("POOR_LEVEL"));	    //13
			varList.add(vpd);
		}
...
//		pd = this.getPageData();
		...
		if(pd!=null) {
			try {
				...
				map.put("学生姓名",pd.get("STU_AGE").toString());	
				...
				startProcessInstanceByKeyHasVariables("KEY_apply_true_lzjxj",map);	//启动流程实例(请假单流程)通过KEY
...
//		pd = this.getPageData();
		...
		if(pd!=null) {
			try {
				...
				map.put("困难等级",pd.getString("POOR_LEVEL"));	
				map.put("USERNAME", Jurisdiction.getUsername());		//指派代理人为当前用户
				startProcessInstanceByKeyHasVariables("KEY_up_college_lzjxj",map);	//启动流程实例(请假单流程)通过KEY
				mv.addObject("ASSIGNEE_",Jurisdiction.getUsername());	//用于给待办人发送新任务消息
				mv.addObject("msg","success");
			} catch (Exception e) {
				mv.addObject("errer","errer");

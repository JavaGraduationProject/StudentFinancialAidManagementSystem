package com.fh.controller.nationgrant.nationgrant;
...
import com.fh.service.nationgrant.nationgrant.NationGrantManager;
...
/**
 * 说明：国家助学金模块 创建人：FH Q313596790 创建时间：2019-02-28
 */
@Controller
@RequestMapping(value = "/nationgrant")
public class NationGrantController extends AcStartController {

	String menuUrl = "nationgrant/list.do"; // 菜单地址(权限用)
	@Resource(name = "nationgrantService")
	private NationGrantManager nationgrantService;
	...
	public ModelAndView save() throws Exception {
		logBefore(logger, Jurisdiction.getUsername() + "新增NationGrant");
		...
		pd = this.getPageData();
		pd.put("NATIONGRANT_ID", this.get32UUID()); // 主键
		nationgrantService.save(pd);
	...
	public void delete(PrintWriter out) throws Exception {
		logBefore(logger, Jurisdiction.getUsername() + "删除NationGrant");
		if (!Jurisdiction.buttonJurisdiction(menuUrl, "del")) {
			return;
		} // 校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		nationgrantService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception {
		logBefore(logger, Jurisdiction.getUsername() + "修改NationGrant");
	...
	public ModelAndView list(Page page) throws Exception {
		logBefore(logger, Jurisdiction.getUsername() + "列表NationGrant");
		...
		page.setPd(pd);
		List<PageData> varList = nationgrantService.list(page); // 列出NationGrant列表
		mv.setViewName("nationgrant/nationgrant/nationgrant_list");
	...
	public ModelAndView listApplyFlow(Page page) throws Exception {
		...
		page.setPd(pd);
		List<PageData> varList = nationgrantService.list(page); // 列出NationGrant列表
		mv.setViewName("nationgrant/nationgrant/nationgrant_list_apply_flow");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		mv.addObject("QX", Jurisdiction.getHC()); // 按钮权限
		return mv;
	...
	public ModelAndView listUpFlow(Page page) throws Exception {
		...
		page.setPd(pd);
		List<PageData> varList = nationgrantService.list(page); // 列出NationGrant列表
		mv.setViewName("nationgrant/nationgrant/nationgrant_list_up_flow");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		mv.addObject("QX", Jurisdiction.getHC()); // 按钮权限
		return mv;
	...
	public ModelAndView goAdd() throws Exception {
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("nationgrant/nationgrant/nationgrant_edit");
	...
	public ModelAndView goEdit() throws Exception {
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = nationgrantService.findById(pd); // 根据ID读取
		mv.setViewName("nationgrant/nationgrant/nationgrant_edit");
	...
	public Object deleteAll() throws Exception {
		logBefore(logger, Jurisdiction.getUsername() + "批量删除NationGrant");
		if (!Jurisdiction.buttonJurisdiction(menuUrl, "del")) {
			return null;
		} // 校验权限
		PageData pd = new PageData();
		...
		if (null != DATA_IDS && !"".equals(DATA_IDS)) {
			...
			nationgrantService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		} else {
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception {
		logBefore(logger, Jurisdiction.getUsername() + "导出NationGrant到excel");
		...
		List<PageData> varOList = nationgrantService.listAll(pd);
...
//		pd = this.getPageData();
		...
		if (pd != null) {
			try {
				...
				startProcessInstanceByKeyHasVariables("KEY_apply_true_gj_zxj", map); // 启动流程实例(请假单流程)通过KEY
...
//		pd = this.getPageData();
		...
		if (pd != null) {
			try {
				...
				map.put("困难等级", pd.getString("POOR_LEVEL"));
				map.put("USERNAME", Jurisdiction.getUsername()); // 指派代理人为当前用户
				startProcessInstanceByKeyHasVariables("KEY_up_college_gj_zxj", map); // 启动流程实例(请假单流程)通过KEY
				mv.addObject("ASSIGNEE_", Jurisdiction.getUsername()); // 用于给待办人发送新任务消息
				mv.addObject("msg", "success");
			} catch (Exception e) {
				mv.addObject("errer", "errer");
